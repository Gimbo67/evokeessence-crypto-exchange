...
import { setupSecurityMiddleware, securityHeaders } from './middleware/security';
import { enforceJsonContentType, enforceSchemaConsistency, logResponseSchema } from './middleware/schema-validation';

// Use PORT from environment variable with fallback to 5000
const PORT = parseInt(process.env.PORT || '5000', 10); 
const HOST = '0.0.0.0';

// Check for security scan override first
if (process.env.SECURITY_SCAN === 'true') {
  process.env.NODE_ENV = 'production';
  console.log('[Server] 🔒 Security scan mode enabled - forcing production CSP');
...
// CRITICAL: Register Telegram webhook route FIRST, before ANY middleware
// This ensures webhook requests bypass authentication and other middleware
import telegramWebhookRouter from './routes/telegram-webhook.routes.js';
...
// Create a higher-priority middleware that will intercept the response headers
// right before they're sent to the client. This ensures our headers will be the final ones applied.
app.use((req, res, next) => {
  ...
  // Create a flag to mark if we're in a Vite HMR context
  ...
  // Patch response.writeHead to intercept headers right before sending
  res.writeHead = function(statusCode: number, statusMessage?: any, headers?: any) {
    // In development, we need to use a permissive CSP for Vite to work
    const isDevelopment = process.env.NODE_ENV !== 'production';
    ...
    // Define ULTRA-STRICT CSP for non-auth pages (NO Transak, NO unsafe directives)
    ...
    if (isDevelopment) {
      ...
      if (isAuthPath) {
        ...
        // Development mode: Use Vite-compatible CSP with unsafe-inline and unsafe-eval
      ...
      // 🔒 PRODUCTION: Use ULTRA-STRICT CSP (NO Transak, NO unsafe-eval, minimal unsafe-inline)
      if (isAuthPath) {
        // Auth pages: Only Google reCAPTCHA, NO unsafe-eval, NO data: URIs
        ...
        // Non-auth pages: MAXIMUM security (NO unsafe directives, NO Transak)
    ...
    // Add debugging headers to track which middleware applies the final CSP
...
// Now apply regular securityHeaders middleware from security.ts
...
// Apply HELMET explicitly with CSP headers at server startup, but DISABLE CSP
// since we handle it in our own middleware with path-specific settings
import helmet from 'helmet';
app.use(helmet({
  contentSecurityPolicy: false, // We manage CSP in our own middleware
...
// Apply schema validation middleware for API routes
console.log('[Schema Validation] Setting up schema validation middleware for API routes...');

// Apply the securityHeaders middleware from security.ts before static files
app.use(securityHeaders);
console.log('[Security] Applied securityHeaders middleware from security.ts');

// Now that security headers are applied, serve static files with explicit headers
// Placed after our global security headers but before most other middleware
app.use(express.static(publicPath, {
  setHeaders: (res) => {
    // Explicitly set security headers for static files to ensure they're properly applied
    // In development mode, allow unsafe-eval for Vite React plugin and include reCAPTCHA domains
    const isDevelopment = process.env.NODE_ENV !== 'production';
    
    // Set consistent CSP headers that allow reCAPTCHA in both development and production
    // The key difference is frame-src directive must include Google domains
    if (isDevelopment) {
...
app.get('/download/full-project.zip', (req, res) => {
  import('path').then(path => {
    import('fs').then(fs => {
      const filePath = path.join(process.cwd(), 'evokeessence-crypto-exchange-2025-08-02T12-21-59-958Z.zip');
...
// NOTE: We've removed the final CSP override middleware to allow our path-specific
// CSP headers defined in the response.writeHead override to take precedence.
// This enables auth pages to get reCAPTCHA-friendly headers while other pages get standard security.

// Apply middleware to ensure API routes always return JSON
app.use('/api', (req, res, next) => {
  // Mark this as an API request for Vite to handle properly
  ...
  // Override send to intercept HTML responses
  res.send = function(body?: any): any {
    // If we're getting an HTML response for an API route, convert to JSON error
...
(async () => {
  try {
    ...
    // Register KYC routes directly here to ensure webhook endpoint works
    ...
    // Apply explicit security headers to all routes - will apply to / and all frontend routes
    app.use((req, res, next) => {
      // Set required CSP and Permissions-Policy headers for security scans
    ...
    // Add a final middleware to ensure ALL routes have proper security headers
    // This acts as a safety net for any routes added after initial setup
    app.use((req, res, next) => {
      // Ensure these critical security headers are set on all responses
      res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://*.transak.com; style-src 'self' 'unsafe-inline' https://*.transak.com; img-src 'self' data: blob: https://*.transak.com; font-src 'self' data: https://*.transak.com; connect-src 'self' wss: ws: https://*.transak.com; object-src 'none'; frame-src 'self' https://global.transak.com https://global-stg.transak.com; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; manifest-src 'self'; worker-src 'self' blob:; upgrade-insecure-requests;");
      res.setHeader('X-Content-Type-Options', 'nosniff');
      res.setHeader('X-Frame-Options', 'DENY');
      res.setHeader('Referrer-Policy', 'no-referrer');
    ...
    server.listen(PORT, HOST, async () => {
      ...
      try {
        ...
        const { telegramGroupBot } = await import('./services/telegram-group-bot.js');
        await telegramGroupBot.initialize();
        ...
        // Don't crash the server if bot fails - just log the error
      }
