...
export function registerBypassRoutes(app: Express) {
  ...
  // Test endpoint to debug deletion issues
  app.get('/test-deletion-debug', async (req: Request, res: Response) => {
    try {
      // Test basic select
      const testUser = await db.query.users.findFirst({
        where: eq(users.id, 132)
      });
      ...
      // Test simple raw SQL
      const rawResult = await db.execute(sql`SELECT id FROM users WHERE id = 132`);
