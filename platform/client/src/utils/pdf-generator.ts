...
export const generateUserDataExportPDF = async (
  ...
  try {
    ...
    // Add verification info without QR code
    doc.setFillColor(220, 220, 220);
    doc.rect(165, 45, 30, 30, 'F');
    doc.setFontSize(8);
    doc.setTextColor(80, 80, 80);
    doc.text('Reference ID', 180, 60, { align: 'center' });
    doc.text(referenceId.split('-').pop() || 'N/A', 180, 65, { align: 'center' });
...
export const generateBankStatementPDF = async (
  ...
  try {
    ...
    // Add simple verification area instead of QR code
    doc.setFillColor(240, 240, 240);
    doc.rect(170, 52, 20, 20, 'F');
    doc.setFontSize(6);
    doc.setTextColor(80, 80, 80);
    doc.text('Statement', 180, 60, { align: 'center' });
    doc.text('Verified', 180, 65, { align: 'center' });
...
export const generateTransactionPDF = async (
  ...
  try {
    ...
    // Add verification info without QR code (simplified for reliability)
    doc.setFontSize(7);
    doc.setTextColor(80, 80, 80);
    doc.text(`Statement ID: ${statementId}`, 170, 69, { align: 'center' });
    doc.text('Verify at evokeessence.com', 170, 74, { align: 'center' });
    ...
    // Add a simple verification text area instead of QR code
    doc.setPage(1);
    doc.setFillColor(240, 240, 240);
    doc.rect(165, 45, 30, 30, 'F');
    doc.setFontSize(8);
    doc.setTextColor(80, 80, 80);
    doc.text('Statement', 180, 55, { align: 'center' });
    doc.text('ID:', 180, 60, { align: 'center' });
    ...
    try {
      ...
      console.warn('Blob URL method failed, trying next method:', blobError);
      ...
      try {
        ...
        console.warn('Direct save method failed, trying next method:', directSaveError);
        ...
        try {
          ...
          console.warn('Data URL method failed, trying final fallback:', dataUrlError);
          ...
          try {
            ...
            console.error('All PDF download methods failed:', newTabError);
