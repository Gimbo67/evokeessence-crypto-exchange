...
const transformUserData = (data: any): User => {
  try {
    ...
    const transformedData: any = {
      ...
      isAdmin: data.isAdmin !== undefined ? data.isAdmin : data.is_admin === true,
      isEmployee: data.isEmployee !== undefined ? data.isEmployee : data.is_employee === true,
      isContractor: data.isContractor !== undefined ? data.isContractor : data.is_contractor === true,
      ...
      twoFactorVerified: data.twoFactorVerified !== undefined ?
        data.twoFactorVerified :
        data.two_factor_verified === true,
      // Add requireTwoFactor property
      requireTwoFactor: data.requireTwoFactor !== undefined ?
        data.requireTwoFactor :
        (data.two_factor_enabled === true && data.two_factor_verified !== true),
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (username: string, password: string, recaptchaToken?: string | null) => {
    try {
      ...
      // Use more reliable login method for production
      try {
        ...
        if (response.ok) {
          ...
          const isEmployee = data.isEmployee === true || data.is_employee === true;
          const isContractor = data.isContractor === true || data.is_contractor === true;
          ...
          // Force a fresh auth check to update context state
