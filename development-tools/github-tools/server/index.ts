...
// Explicitly set NODE_ENV to development if not set
...
app.use((req, res, next) => {
  ...
  res.writeHead = function(statusCode: number, statusMessage?: any, headers?: any) {
    ...
    const cspStandard = `default-src 'self'; 
      ...
      object-src 'none'; 
      frame-src 'self' https://global.transak.com https://global-stg.transak.com; 
      base-uri 'none'; 
      form-action 'self'; 
      frame-ancestors 'none'; 
      manifest-src 'self'; 
    ...
    if (isDevelopment) {
      ...
      if (isAuthPath) {
        ...
        this.setHeader('Content-Security-Policy', cspStandard);
        console.log(`[Security-Final-Override] Applied standard CSP to path: ${req.path}`);
      }
    } else {
      // In production, use strict A+ rating CSP that still allows Google reCAPTCHA for auth paths
      if (isAuthPath) {
        ...
        console.log(`[Security-Final-Override] Applied production auth CSP to path: ${req.path}`);
      } else {
        ...
        console.log(`[Security-Final-Override] Applied production standard CSP to path: ${req.path}`);
      }
    }
