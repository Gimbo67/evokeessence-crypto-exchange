  ...
  app.post("/api/deposits", async (req, res) => {
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        // Check for both referral code and direct contractor association
        let userReferralCode = referralCode;
        let userContractorId: number | undefined = undefined;
        
        // Get the user's full profile including contractor_id and referred_by
        ...
        // Special handling for users registered with code A64S (andreavass)
        // This ensures all deposits from this user go to andreavass
        if (userInfo?.referred_by === 'A64S') {
          console.log(`[API_DEPOSIT_CREATE] User registered with special code A64S (andreavass)`);
          
          // If we don't have a contractor_id yet, find andreavass's ID
          if (!userInfo?.contractor_id) {
            console.log(`[API_DEPOSIT_CREATE] Looking up andreavass contractor ID`);
            const andreavass = await tx.query.users.findFirst({
              where: eq(users.username, 'andreavass')
            });
            
            if (andreavass) {
              userContractorId = andreavass.id;
              console.log(`[API_DEPOSIT_CREATE] Found andreavass with ID: ${userContractorId}`);
              
              // Update the user's contractor_id for future deposits
              await tx.update(users)
                .set({ contractor_id: userContractorId })
                .where(eq(users.id, req.user!.id));
                
              console.log(`[API_DEPOSIT_CREATE] Updated user's contractor_id to ${userContractorId}`);
            }
          } else {
            userContractorId = userInfo.contractor_id;
            console.log(`[API_DEPOSIT_CREATE] User already has contractor_id: ${userContractorId}`);
          }
          
          // Always use A64S as the referral code for these users
          userReferralCode = 'A64S';
        } 
        // If no special case, get the referred_by code if there's no explicit referral code
        else if (!userReferralCode) {
          userReferralCode = userInfo?.referred_by || undefined;
          userContractorId = userInfo?.contractor_id;
          
          if (userReferralCode) {
            console.log(`[API_DEPOSIT_CREATE] Using user's saved referral code: ${userReferralCode}`);
          }
          
          if (userContractorId) {
            console.log(`[API_DEPOSIT_CREATE] User has contractor_id: ${userContractorId}`);
          }
        }

        // Calculate deposit with commission and convert to user's currency
        const depositCalc = await calculateDepositAmounts(
          amount, 
        ...
        // Generate unique reference number with specific format that never changes once created
        // Using UUID part for uniqueness instead of timestamp to ensure consistency
        ...
        console.log(`[API_DEPOSIT_CREATE] Generated unique reference: ${reference}`);
        ...
        const [deposit] = await tx
          ...
          .values({
            ...
            referralCode: depositCalc.referralCode,
            contractorId: depositCalc.contractorId,
            contractorCommission: depositCalc.contractorCommission?.toString(),
            createdAt: new Date(),
          })
          .returning();
    ...
    try {
      ...
      const contractorsWithReferrals = await Promise.all(
        contractors.map(async (contractor) => {
          ...
          const clientIds = [...new Set(referralDeposits.map(d => d.userId))];
