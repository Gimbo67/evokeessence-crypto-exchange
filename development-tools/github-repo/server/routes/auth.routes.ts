...
passport.deserializeUser(async (id: number, done) => {
  try {
    ...
    // Log raw PostgreSQL values for debugging
    ...
    const isContractor = user.is_contractor === true || 
                         user.is_contractor === 't' || 
                         user.is_contractor === 1 || 
                         String(user.is_contractor).toLowerCase() === 'true' || 
                         String(user.is_contractor).toLowerCase() === 't';
    ...
    const userWithoutPassword = {
      ...
      username: user.username,
      email: user.email,
      is_admin: isAdmin, // Properly converted boolean
      is_employee: isEmployee, // Properly converted boolean
      is_contractor: isContractor, // Properly converted boolean
      user_group: user.user_group,
      kyc_status: user.kyc_status || 'not_started',
      status: user.status,
    ...
    console.log('User deserialized from auth.routes.ts:', {
      id: userWithoutPassword.id,
      username: userWithoutPassword.username,
      is_admin: userWithoutPassword.is_admin,
      is_employee: userWithoutPassword.is_employee,
      is_contractor: userWithoutPassword.is_contractor,
      user_group: userWithoutPassword.user_group,
      referral_code: userWithoutPassword.referral_code,
      contractor_commission_rate: userWithoutPassword.contractor_commission_rate
    });
  ...
  try {
    ...
    if (validatedData.referred_by) {
      ...
      if (validatedData.referred_by === 'A64S') {
        ...
        console.log(`Checking referral code: ${validatedData.referred_by}`);
        referringContractor = await db.query.users.findFirst({
      ...
      if (!referringContractor) {
        console.log(`Invalid referral code: ${validatedData.referred_by}`);
      } else if (!referringContractor.is_contractor) {
        console.log(`User with referral code ${validatedData.referred_by} is not a contractor`);
        referringContractor = null;
        contractorId = null;
      } else {
        console.log(`Valid referral from contractor: ${referringContractor.username} (ID: ${referringContractor.id})`);
        contractorId = referringContractor.id;
      }
    }
    ...
    const newUser = {
      ...
      referred_by: referringContractor ? validatedData.referred_by : null,
