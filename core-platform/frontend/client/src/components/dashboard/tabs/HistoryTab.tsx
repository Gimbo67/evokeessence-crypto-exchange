...
import { generateSimpleTransactionPDF } from "@/utils/pdf-generator-simple";
...
export default function HistoryTab({ user }: HistoryTabProps) {
  ...
  const handleDownloadPdf = async () => {
    if (preparedTransactions.length === 0) {
      console.log('[HistoryTab] No transactions available for PDF generation');
      return;
    }
    
    try {
      setIsGeneratingPdf(true);
      console.log('[HistoryTab] Starting PDF generation process...');
      console.log('[HistoryTab] Transaction count:', preparedTransactions.length);
      console.log('[HistoryTab] User data:', { id: user.id, username: user.username });
      ...
      console.log('[HistoryTab] Prepared user data:', userData);
      
      // Get the currently selected language from the language context
      const currentLanguage = localStorage.getItem('language') || 'en';
      console.log('[HistoryTab] Using language:', currentLanguage);
      
      // Log sample transaction data
      console.log('[HistoryTab] Sample transaction:', preparedTransactions[0]);
      
      // Generate the PDF with language parameter for proper localization
      // Using await to properly handle the async function
      console.log('[HistoryTab] Calling generateSimpleTransactionPDF...');
      await generateSimpleTransactionPDF(preparedTransactions, userData, t, currentLanguage);
      
      // Log successful PDF generation for analytics
      console.log(`[HistoryTab] SUCCESS: Transaction history PDF generated in ${currentLanguage} language for user ID: ${user.id}`);
    } catch (error) {
      console.error('[HistoryTab] Error generating PDF:', error);
      console.error('[HistoryTab] Error details:', {
      ...
      console.log('[HistoryTab] PDF generation process completed');
